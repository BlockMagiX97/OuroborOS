...
void abort_print_state(struct regs *regs) {

}
struct regs *handle_exceptions(struct regs *regs) {
	switch (regs->int_no) {
		default:
	}
}
struct regs *isr_dispatch(struct regs *regs) {
	if (regs->int_no < 32) {
		return handle_exceptions(regs);
	}
	return regs;
}
